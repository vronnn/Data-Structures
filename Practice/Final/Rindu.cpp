...
int distanceFromRoot(nodeptr root, int x)
{
    if (root->data == x)
        return 0;
    else if (root->data > x){
        return  abs(root->left->data - root->data)+ distanceFromRoot(root->left, x);
    }
    return abs(root->right->data - root->data) + distanceFromRoot(root->right, x);
}

int distanceBetween2(nodeptr root, int a, int b)
{
    if (root == NULL)
        return 0;
...
int findDistWrapper(avlptr avl, int a, int b)
{
    if (a > b)
        swap(a, b);
    return distanceBetween2(avl->root, a, b);
}

int main(){
    avlptr jalan = init_avl();
    int n,x,a,b;
    cin >> n;
    while(n--){
        cin >> x;
    ...
    cin >> a >> b;
    cout << findDistWrapper(jalan, a, b);
}
