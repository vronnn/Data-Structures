...
void print_selisih(avlptr avl, int value){
    nodeptr temp = cari_ayah(avl->root, value);
    if(temp == NULL) printf("Aku adalah Kepala Keluarga Pawel\n");
    else{
        nodeptr kakek = cari_ayah(avl->root, temp->data);
        if(kakek == NULL) printf("%d\n", avl->root->data);
        else{
            if(kakek->left && kakek->left->data == temp->data){
                printf("%d\n", (kakek->right)? kakek->right->data - kakek->left->data : kakek->left->data);
            }
            if(kakek->right && kakek->right->data == temp->data){
                printf("%d\n", (kakek->left)? kakek->right->data - kakek->left->data : kakek->right->data);
            }
        }
    }
}
