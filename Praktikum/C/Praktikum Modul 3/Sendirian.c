...
int main(){
    ...
    for(i = 0; i < n; i++){
        arr[i] = hew[i];
        if(!find_avl(kandang, arr[i])){
            insert_avl(kandang, arr[i]);
            }
        else remove_avl(kandang, arr[i]);
    }
    if(isEmpty_avl(kandang)) printf("aman bang\n");
    else inorder(kandang);
}
